...
public class GraphPanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        Node selected = null;
        for(Node node : graph.getNodes()) {
            if(DrawUtils.isWithinBounds(e, node.getCoordinates())){
                selected = node;
                break;
            }
        }
        ...
        if(hoveredEdge!=null){
            ...
            String input = JOptionPane.showInputDialog("Atribuie un cost pentru " + hoveredEdge.toString()
                    + " : ");
            try {
                ...
                if (weight > 0) {
                    ...
                    JOptionPane.showMessageDialog(null, "Costurile trebuie sa fie pozitive");
                }
        ...
        for(Node node : graph.getNodes()) {
            if(DrawUtils.isOverlapping(e, node.getCoordinates())){
                JOptionPane.showMessageDialog(null, "Nu pot exista doua noduri in acelasi loc");
                return;
            }
        }
    ...
    public void mouseReleased(MouseEvent e) {
        for (Node node : graph.getNodes()) {
            if(selectedNode !=null && node!= selectedNode && DrawUtils.isWithinBounds(e, node.getCoordinates())){
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        for (Node node : graph.getNodes()) {
            if(selectedNode ==null && DrawUtils.isWithinBounds(e, node.getCoordinates())){
                selectedNode = node;
            } else if(DrawUtils.isWithinBounds(e, node.getCoordinates())) {
                hoveredNode = node;
            }
        }
        ...
        if(selectedNode !=null){
            if(e.isControlDown()){
                selectedNode.setCoordinates(e.getX(), e.getY());
                cursor = null;
                repaint();
                return;
            }

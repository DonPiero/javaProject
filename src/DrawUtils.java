...
public class DrawUtils {
    ...
    public static boolean isOnEdge(MouseEvent e, Edge edge) {
        ...
        int dist = distToSegment( e.getPoint(),
                edge.getNodeOne().getCoordinates(),
                edge.getNodeTwo().getCoordinates() );
        if (dist<6)
            return true;
        return false;
    }
    ...
    public void drawWeight(Edge edge) {
        Point from = edge.getNodeOne().getCoordinates();
        Point to = edge.getNodeTwo().getCoordinates();
        int x = (from.x + to.x)/2;
        int y = (from.y + to.y)/2;
